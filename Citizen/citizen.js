...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchRequestData() {
        ...
        xhr.open('GET', 'fetch_citizen_requests.php/' + user_id, true);
    ...
    function fetchOfferData() {
        ...
        xhr.open('GET', 'fetch_citizen_offers.php/' + userID, true);
    ...
    function fetchCategories() {
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                offerItems = JSON.parse(xhr.responseText);
                populateRequestCategories(offerItems);
            } else {
                console.error('Failed to fetch data');
            }
        };
...
function populateRequestItems() {
    const newRequestTable = document.getElementById('newAithmata2');
    ...
    if (selectedCategory === 'all') {
        ...
        const filteredItems = allItems.filter(item => item.CategoryID === selectedCategory);
